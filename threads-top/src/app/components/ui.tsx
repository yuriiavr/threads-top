...
import { MessageCircle, Heart, TrendingUp, Menu, X, Clock } from "lucide-react";
...
export function PostCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative p-6 md:p-7 bg-[#080808] border border-zinc-900 rounded-2xl flex gap-6 items-start transition-all duration-300 group-hover:bg-[#0a0a0a] group-hover:border-zinc-800/80 group-hover:shadow-2xl ${c.shadow} card-body`}
      >
        <div className="flex flex-col items-center pt-1 w-[40px] flex-shrink-0">
          <span
            className={`text-xl font-bold font-mono text-zinc-800 ${c.rankHover} transition-colors rank-number`}
          >
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="flex items-center justify-between mb-4 min-w-0">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div
                className={`w-9 h-9 flex-shrink-0 rounded-full bg-zinc-900 border border-zinc-800 ${c.avatar} flex items-center justify-center transition-colors text-xs font-bold text-zinc-400 ${c.avatarText} uppercase avatar-box`}
              >
            ...
            {(showDate || post.first_seen_at) && (
              <span className="text-[11px] text-zinc-600 font-mono hidden sm:block ml-2 flex-shrink-0">
                {daysAgo(post.first_seen_at as string)}
              </span>
            )}
          </div>
          <p
            className={`${isHall ? "text-lg md:text-xl font-medium italic" : "text-[16px] md:text-[17px] font-normal"} leading-[1.65] text-zinc-400 group-hover:text-zinc-100 transition-colors mb-6 break-words post-caption`}
          >
            {isHall && '"'}
...
export function MobileSnapList({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div ref={listRef} className="mobile-snap-list md:hidden">
      {children}
    </div>
  );
}
...
export function SortControl({
  ...
  current: "likes" | "recent";
  onChange: (v: "likes" | "recent") => void;
  accent: AccentColor;
}) {
  const c = colors[accent];
  return (
    <div className="flex w-full p-1 bg-zinc-900 border border-zinc-800 rounded-xl md:rounded-lg md:w-auto">
      <button
        onClick={() => onChange("likes")}
        className={`flex flex-1 justify-center items-center gap-2 px-4 py-2 rounded-lg md:px-3 md:py-1.5 transition-all text-sm font-bold ${
          current === "likes"
            ? `bg-zinc-800 ${c.badge}`
      ...
      >
        <TrendingUp size={16} />
        <span className="md:hidden lg:inline">Top</span>
      </button>
      <button
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-zinc-100 font-sans antialiased overflow-x-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute -top-[10%] -left-[10%] w-[50%] h-[50%] ${glow} blur-[130px] rounded-full`}
        />
        ...
        <div className="flex flex-col gap-8 border-b border-zinc-900/50 pb-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex flex-col md:items-end p-4 rounded-xl bg-zinc-900/20 border border-zinc-800/30 backdrop-blur-sm">
              ...
              <p className="text-lg font-mono text-zinc-300 font-medium">
                {periodLabel}
              </p>
            </div>
          </div>
          <nav className="flex flex-wrap gap-3 items-center">{nav}</nav>
        </div>
      </header>
      ...
      <main className="relative max-w-4xl mx-auto px-6 pb-32 hidden md:block">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40 space-y-6">
            <div
              className={`w-10 h-10 border-2 border-zinc-800 ${c.spinner} rounded-full animate-spin`}
            />
            <p className="text-xs text-zinc-600 tracking-[0.4em] uppercase font-medium">
              {loadingText}
            </p>
          </div>
        ) : (
